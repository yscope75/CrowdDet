...
class WiderPerson(torch.utils.data.Dataset):
    def __init__(self, config, if_train):
        ...
        self.records = misc_utils.load_lines(source)
        self.config = config
    ...
    def load_record(self, record):
        if self.training:
            ...
            if_flap = False
        # image
        image_path = os.path.join(self.config.image_folder, record+'.jpg')
        ...
        if self.training:
            # ground_truth
            gtboxes = misc_utils.load_bb(self.config.annotations_root,record)
            ...
            return image, gtboxes, im_info, record
